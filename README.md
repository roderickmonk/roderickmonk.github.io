...
## Starting RabbitMq
```bash
  docker run -d --hostname my-rabbit -v ~/rabbitmq:/var/lib/rabbitmq -p 5672:5672 -p 15672:15672 --name rabbitmq rabbitmq:3-management
```

## Pushing a docker image to Docker Hub
```bash
  docker login -u "roderickmonk" -p "2620Turner" docker.io
  docker tag aws-gateway:latest roderickmonk/aws-gateway:latest
  docker push roderickmonk/aws-gateway:latest
```

## Building
```bash
    npx electron-builder --linux deb # Building for Debian (Ubuntu)
